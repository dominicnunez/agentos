...
| Repository change control | Active [`Protect main`](REPOSITORY_CHANGE_CONTROL.md) ruleset requires pull requests, current-head exact CI checks, and resolved conversations; blocks deletion and force pushes; and has a documented administrator recovery boundary | PARTIAL | Retain a conforming post-enforcement merge and a controlled fail-closed rejection using a non-bypass actor, then review effectiveness under [Issue #127](https://github.com/dominicnunez/agentos/issues/127) |
