...
| Model completion review | Agent self-certification, stale review, result disclosure, ephemeral-origin loss, unbounded history scans | owner-only private control; exact candidate/evidence fingerprint; no-store responses; durable idempotent decision and recovery for root and locally reviewable child Tasks; tenant/type/limit applied in SQLite before exact stream validation | the user's subjective judgment can be wrong |
...
| Exact-effect approval | approval through chat, changed effect, stale or expired decision, ephemeral-origin loss | owner-only private control; full ledger-sourced effect view; typed confirmation; immutable fingerprint; revalidation on every transition and at transactional use; bounded authorized terminal-decision projection | a compromised owner can approve within that account's V1 authority |
