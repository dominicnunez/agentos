...
| Organizational knowledge | forged authorship or validator kind, internal/A2A Agent conflation, self-validation, future or cross-tenant evidence, lineage omission, revoked validation authority, pagination races, live/recovery validator drift | distinct `AGENT` and `EXTERNAL_AGENT` kinds; creator provenance bound to authenticated gateways or exact admitted Agent executions; exact validator-kind capability traces with transactional expiry/revocation/freeze recheck; nonempty exact active lineage for derived knowledge; one-snapshot retrieval; one shared Event-Contract validator with verified lease history for startup materialization and offline verification | validly authorized user or independent-Agent judgment can still be mistaken; knowledge remains untrusted context |
