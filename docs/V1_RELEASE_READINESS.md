...
| Release artifact pipeline | READY | A separate read-only workflow builds Linux, macOS, and Windows artifacts twice, checks byte reproducibility, verifies both commands, and emits checksums, per-target CycloneDX SBOMs, plus deterministic in-toto provenance. Nothing is uploaded or published. |
