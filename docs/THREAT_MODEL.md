...
| Semantic intake | invented operator choices, hidden Goal substitution, confirmation replay, Goal lifecycle race | strict bounded output; explicit source-message provenance for Goal IDs; exact-ID presence check; active same-tenant Goal admission in the confirmation transaction; complete draft fingerprint and explicit confirmation; immutable Intent and Work Goal binding; later Goal changes cannot strand admitted Work | an authorized operator can explicitly select the wrong active Goal |
| Task-DAG planning | prompt injection, authority invention, graph bombs, dependency cycles, partial persistence, mislabeled lifecycle state, internal-task disclosure | exact accepted-Intent fingerprint; bounded closed-schema output; runtime-owned root; execution-kind allowlist; 16-Task ceiling; deterministic handler registry; cycle and dependency validation; atomic graph commit; immutable Task contracts; exact event/status transitions at write, replay, and recovery; root-only A2A lookup | a valid but poor Plan can waste bounded model work or require independent review |
| Agent dispatch | deactivation race, stale or substituted Agent configuration, cross-tenant roster reference, replayed start authority | one SQLite transaction reloads the exact active Agent, blueprint, and execution profile while consuming the pending Task revision; sealed start detail binds projection versions and admission events; replay and recovery reject stale, superseded, malformed, or cross-organization bindings; the binding grants no capability or effect authority | a dispatch committed before deactivation may finish; interrupted adaptive provider contact remains uncertain |
...
| SQLite and recovery | corruption, forged projection-shaped events, copied or orphaned admission, identity or tenant substitution, mislabeled Agent, Mission, Goal, Work, or Task state, missing terminal evidence, partial state, unsafe overwrite, unauthorized access | append-only events and versioned records; one-to-one event-coupled projection fingerprints; reserved projection keys and lifecycle labels; exact identity, parent, correlation, prior/resulting state, and terminal Work/Goal evidence validation during write, replay, startup, backup, and restore; read-only integrity/schema verification; no-overwrite backup and restore; private data paths; service umask `0077` | host file access can reveal or alter data; storage encryption and cryptographic ledger attestation are external |
