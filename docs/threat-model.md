...
This threat model describes the codebase at the revision containing this document, including its CI controls. Implemented controls, incomplete controls, and prerequisites for unsupported execution mechanisms are distinguished below. A passing test is evidence for a control, not proof of comprehensive security.
...
The private owner socket exposes organization freeze inspection and mutation through `/v1/control/freeze`. The service binds the configured organization and human owner after kernel peer-UID authentication; request bodies, headers, notifications, and capability references cannot establish release authority. The dashboard bridge and A2A listener do not expose this endpoint. Closed JSON decoding and request-size limits reject missing, unknown, duplicate, null, and oversized command fields.

The typed writer compares the current event reference and version in its transaction, records the human actor and exact predecessor, and commits before signalling local contexts. A newer hold invalidates an older release. An exact retry of the latest committed command is idempotent and does not append another event. Release permits fresh independently admitted work; it never revives cancelled contexts or automatically resumes suspended Tasks. Generic freeze writes are reserved. Live authority reads and full replay apply the same validation to the organization's complete freeze history, preserving metadata-free legacy records while rejecting invalid actor, event, predecessor, transition, and evidence-removal relationships. Cold admission validates the complete history before registering live work. Storage v11 adds organization-scoped mutation and rewrite tokens maintained by fingerprinted triggers over freeze records and related events. Live observations read these tokens and any appended suffix in one transaction, reuse an immutable validated prefix for unchanged history, and validate new admissions with the same rules as full replay. Runtime snapshots reuse validated freeze history within the exact database transaction, including inbox polling, knowledge authority reads, and inference validation; each reuse checks freshness tokens so even same-transaction mutations invalidate old proof. Uncommitted transaction views never enter the shared live cache. Rewrites and schema changes invalidate that proof; live work fails closed unless a fresh validated view is available. Status reads and the typed writer can rebuild the proof, and a writer publishes its candidate only after commit. Active organizations retain a shared cached view, with at most eight inactive views retained. Cold validation and copying changed history grow with history size; idle polls do not replay it. The tokens are freshness evidence under the trusted local-storage assumption, not protection against an operator who can forge the database and its metadata. Unavailable or timed-out containment reads never renew the independent watchdog.
...
Task stop requests now atomically suspend the exact running Task before waiting for its handler to return. A bounded acknowledgement wait records uncertainty without claiming a finished execution, releases the submission service for unrelated work, and permits a later handler return to commit local completion evidence and usage together. The stop request binds the original cause and execution start; a later release or hold cannot change that cause or reopen the Task. Typed writers reserve stop lifecycle events and reject new computation and ordinary publication for the stopped execution. Full replay validates the same event bindings and ordering. Runtime cancellation stops adaptive planning and active Task supervisors during startup recovery and before HTTP request draining; a listener failure also stops active work before draining other listeners. HTTP request contexts receive shutdown cancellation, including active normalization, while bounded accounting bookkeeping remains permitted. Task result admission remains cancellable until completion verification commits; finishing an already-verified Task projection does not reverse that durable decision. Shutdown closes owned providers and attempts a bounded drain before closing the ledger. A successful drain joins owned calls and late writes; a timeout is reported and retains durable uncertainty and conservative accounting, with no guarantee of recording returns after shutdown. If the durable writer remains unavailable, the runtime reports the failure; it cannot promise persistence during a storage outage.

Adaptive planning and model-based intent normalization now have separate typed stop records bound to their exact context manifests. The runtime tracks these operations through durable plan, draft, or ordinary-failure admission. Cancellation records stop intent before waiting for a blocked callback, releases its caller after bounded bookkeeping, and retains uncertainty until a local return permits atomic usage and confirmation. A call prevented from starting receives explicit `NOT_STARTED` evidence without a fabricated return time, provider acknowledgement, or inference-guard not-sent claim. A prior committed result or failure remains authoritative. Fresh attempts after proven non-dispatch, and fresh normalization after confirmed non-security cancellation, still require current admission, routing, and budget checks; uncertain or security-held invoked attempts remain latched absent separate guard proof of non-dispatch. Live admission and full replay reject ordinary post-stop activity and contradictory stop evidence. Local return and provider-side termination remain separate claims.

The Codex adapter distinguishes a matching terminal turn notification from a mere interrupt acknowledgement. If cooperative stop remains unconfirmed, it attempts to terminate its owned local process tree without waiting for the current turn's serialization permit. A hard-stop attempt retires the old process and transport without starting more work for the stopped request. A later independent call initializes a fresh client in a new isolated directory under its own context. The interrupted request is never replayed by this replacement, and its uncertain accounting and stop evidence remain intact. Failed recreation leaves the old runtime unavailable; deliberate adapter shutdown prevents recreation. Local turn termination, attempted process termination, observed process termination, and remote uncertainty are separate evidence. Replacing a connection cannot confirm that the old process or remote work stopped. HTTP request cancellation and local process exit cannot prove remote termination.

Active containment remains partial. The complete coordination/output and external-effect audit, and complete dispatch/cancellation timelines remain unfinished under issue #178. Suspended Tasks remain latched after release and restart; a general Task-resumption capability is deferred rather than required to clear that latch automatically. The owner mutation and durable stop records still depend on the ledger writer. Arbitrary noncooperative handlers and remote providers have no general forced-termination guarantee. Agent- and Work-scoped quarantine is not claimed.
...
Incident validation follows projection dependencies in both directions, including incoming Task parent/dependency links and foreign-tenant references to globally keyed records, and checks retained lifecycle admissions and completion evidence with shared recovery validators. Selected capability grants/revocations and connection-policy replacements are included when validating authority and inference admissions. Inference admission and completion checks reconstruct eligible inbox and factual-context candidates independently of manifest references, using the retained execution boundary. Linked intake messages and private Task dependencies include their terminal lifecycle evidence. Exact record backing and aggregate loading bounds, including nested references, constrain that supporting evidence; it does not enter the public report. Events in the selected Work correlation must remain in the public Work history. A history can exceed those bounds even when the selected timeline is small. Complete-chain verification still scans retained ledger history per request.
