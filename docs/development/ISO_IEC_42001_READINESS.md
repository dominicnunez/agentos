...
| Documented information | Versioned repository, release source/provenance controls, startup/recovery-verified cryptographic event-chain evidence, and a closed hash-bound public AIMS document manifest with fail-closed CI verification | PARTIAL | Approve the draft records, establish confidential evidence and external-document controls, operate retention and review, and approve an external checkpoint/retention procedure |
| Repository change control | Active [`Protect main`](REPOSITORY_CHANGE_CONTROL.md) ruleset requires pull requests, current-head exact CI checks, and resolved conversations; blocks deletion and force pushes; has a documented administrator recovery boundary; and has one conforming exact-head merge recorded in [PR #138](https://github.com/dominicnunez/agentos/pull/138) | PARTIAL | Monitor operation and review effectiveness. A controlled non-bypass rejection remains untested by accepted decision in closed [Issue #127](https://github.com/dominicnunez/agentos/issues/127); reopen it if a suitable identity becomes available or observed behavior conflicts with policy |
