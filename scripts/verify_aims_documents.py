...
OUTCOME_DOCUMENT_IDS = {
    "audit": "aims.audit-result",
    "management_review": "aims.management-review-result",
...
def _parse_manifest_bytes(data: bytes, label: str) -> dict[str, Any]:
    ...
    if type(schema_version) is not int or schema_version not in MANIFEST_KEYS_BY_VERSION:
        raise VerificationError(
            f"{label}.schema_version must be one of {sorted(MANIFEST_KEYS_BY_VERSION)}"
        )
    _require_exact_keys(manifest, MANIFEST_KEYS_BY_VERSION[schema_version], label)
    ...
    if schema_version == 2:
        _validate_assessment_outcomes(manifest["assessment_outcomes"], label)
    return manifest
...
def _validate_assessment_outcomes(value: Any, label: str) -> None:
    if not isinstance(value, dict):
        raise VerificationError(f"{label}.assessment_outcomes must be an object")
    _require_exact_keys(value, ASSESSMENT_OUTCOME_KEYS, f"{label}.assessment_outcomes")
    expected = {
    ...
    for outcome_name, (keys, result_key, allowed) in expected.items():
        ...
        outcome_label = f"{label}.assessment_outcomes.{outcome_name}"
        ...
        if outcome_name == "audit":
            count = outcome["open_blocking_findings"]
...
def verify_history(
    ...
    prior_schema = prior_manifest["schema_version"]
    current_schema = manifest["schema_version"]
    if current_schema < prior_schema:
        raise VerificationError("AIMS manifest schema version cannot decrease")
    prior_outcomes = prior_manifest.get("assessment_outcomes")
    current_outcomes = manifest.get("assessment_outcomes")
    if current_outcomes != prior_outcomes:
        if current_outcomes is None:
            raise VerificationError("assessment outcomes cannot be removed")
...
def verify_history_bytes(
    ...
    manifest = _parse_manifest_bytes(manifest_bytes, "AIMS manifest")
    verify_history(prior_manifest, manifest)
    return manifest
