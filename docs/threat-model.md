...
This is partial active containment, not comprehensive quarantine. Complete suspended-task reconciliation/resumption, independent abort reliability, the complete coordination/output and external-effect audit, and complete dispatch/cancellation timelines and historical enforcement remain unfinished under issue #178. The owner mutation still depends on the ordinary ledger writer; it does not implement an independent durable abort/acknowledgement path when that writer or a noncooperative handler fails. Execution-, Agent-, and Work-scoped quarantine is not claimed.
