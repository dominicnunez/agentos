...
the reviewed layout fingerprint covers the index. Stop validation remains a
separate live-admission and replay requirement, not an implication of the index.

Storage v11 adds organization-scoped freeze observation tokens and reviewed
triggers covering freeze records and their admission events. Migration seeds
tokens for existing histories without rewriting their records or events. These
tokens identify unchanged or append-only snapshots; they do not replace history
validation or the stored-byte integrity chain. The schema fingerprint includes
the exact trigger definitions. Runtime startup and offline verification reject
missing or altered triggers. Live containment validates cold history before
registration, then uses these tokens to reuse its validated prefix and check new
admissions. A rewritten history or schema change invalidates the live proof;
