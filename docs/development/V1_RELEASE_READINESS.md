...
| Required CI | PARTIAL | Build, vet, lint, race tests, vulnerability scanning, architecture checks, interoperability, and advisory dead-code analysis run successfully on pushes and pull requests. GitHub does not yet enforce those checks through `main` branch protection or a repository ruleset; the governed control decision and enforcement test remain open in [Issue #127](https://github.com/dominicnunez/agentos/issues/127). |
...
| Full installed Linux test | PASS | Release CI installs the packaged Linux amd64 binary into clean system and ordinary-user layouts on a disposable host; verifies exact paths, modes, ownership, resumable state, diagnostics, private UID access, header-impersonation resistance, restart continuity, and packaged backup/verify/restore; and proves the production binary rejects an unsupported provider before storage creation. The real provider type uses a non-working local credential and both services are OS-confined from IP networking, so no provider is contacted. GitHub-hosted CI has no stable logged-in user systemd manager: user-mode service persistence is therefore exercised by restarting the installed process in an isolated network namespace, while user-unit generation and activation commands remain separately tested. Live provider replacement remains under its separate approval gate. |
