...
[V1_ACCEPTANCE_STATUS.md](v1-acceptance-status.md). Passing the architecture
...
| Controlled startup | PASS | The runtime refuses incomplete configuration and requires validated real providers with explicit routing for named connections. A2A remains disabled without a reviewed Agent registry. |
...
| Five-minute governed workflow | PARTIAL | The user-facing [quickstart](../guides/quickstart.md) and Linux dashboard-loop test cover durable direction, reviewed work, bounded execution, result recovery, and organization projection. Validate the released binary on a clean user-mode Linux installation after provider live-test approval. |
| Full installed Linux test | PASS | Release CI installs the packaged Linux amd64 binary into clean system and ordinary-user layouts on a disposable host; verifies exact paths, modes, ownership, resumable state, diagnostics, private UID access, header-impersonation resistance, restart continuity, and packaged backup/verify/restore; and proves the production binary rejects an unsupported provider before storage creation. System mode starts the generated systemd units with a production-encrypted credential. The hosted Ubuntu 24.04 runner exercises the installed user process offline because systemd 255 is below the documented systemd 258 minimum. A separate Ubuntu 26.04 WSL acceptance run recorded in [Issue #135](https://github.com/dominicnunez/agentos/issues/135), using systemd 259 and Go 1.26.6, created a user-scoped encrypted provider credential and successfully started, stopped, and restarted the generated user unit while preserving durable state. Live provider replacement remains under its separate approval gate. |
