...
Task stop requests now atomically suspend the exact running Task before waiting for its handler to return. A bounded acknowledgement wait records uncertainty without claiming a finished execution, releases the submission service for unrelated work, and permits a later handler return to commit local completion evidence and usage together. The stop request binds the original cause and execution start; a later release or hold cannot change that cause or reopen the Task. Typed writers reserve stop lifecycle events and reject new computation and ordinary publication for the stopped execution. Full replay validates the same event bindings and ordering. Shutdown cancels active Task supervisors before closing owned providers and boundedly draining late stop writes. If the durable writer remains unavailable, the runtime reports the failure; it cannot promise persistence during a storage outage.

The Codex adapter distinguishes a matching terminal turn notification from a mere interrupt acknowledgement. If cooperative stop remains unconfirmed, it attempts to terminate its owned local process tree without waiting for the current turn's serialization permit. A hard-stop attempt permanently closes that shared adapter to later calls, even when process termination cannot be confirmed. Local turn termination, attempted process termination, observed process termination, and remote uncertainty are separate evidence. Terminating the shared process may interrupt other calls using that connection. HTTP request cancellation and local process exit cannot prove that remote provider work stopped.
