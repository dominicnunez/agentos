...
| SQLite and recovery | corruption, forged projection-shaped events, copied or orphaned admission, mislabeled Agent, Goal, or Task state, partial state, unsafe overwrite, unauthorized access | append-only events and versioned records; one-to-one event-coupled projection fingerprints; reserved projection keys and lifecycle labels; exact prior/resulting state and terminal Goal-evidence validation during write, replay, startup, backup, and restore; read-only integrity/schema verification; no-overwrite backup and restore; private data paths; service umask `0077` | host file access can reveal or alter data; storage encryption and cryptographic ledger attestation are external |
