...
| Documented information | Versioned repository, release source/provenance controls, immutable handoff, a startup/recovery-verified cryptographic event chain, and signed external checkpoint, key-transition, and reviewed-recovery evidence | PARTIAL | Document owner, approval, retention, review, supersession, external-document controls, and an operated checkpoint/transition retention procedure |
...
| Security and resilience | Threat model, authentication/authorization, private user socket, A2A isolation, independently signed ledger-head checkpoints, fail-closed rollback/substitution detection, reviewed key and crash recovery, paired backup/restore tests, and bounded deterministic incident replay from a verified snapshot | PARTIAL | Operational security baselines, key-custody and recovery exercises, recovery objectives, and deployment evidence |
...
| Incident and corrective action | Fail-closed runtime behavior, issue tracking, signed tamper-evident event-head evidence, explicit trust-discontinuity and ambiguous-commit recovery records, and payload-free deterministic Work replay | PARTIAL | Operator-owned AI incident/nonconformity record, containment, root cause, corrective action, effectiveness review, and lessons learned |
...
companion, which verifies the exact JSON bytes. It is separate from the signed
external ledger checkpoint and does not by itself attest to ledger integrity,
control effectiveness, or conformity.
