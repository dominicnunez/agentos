...
| AIMS evidence export | cross-tenant disclosure, unbounded ledger dump, prompt/result/credential leakage, false certification or control-effectiveness claim, tampered downloaded artifact | authenticated local-owner capability; exact read-only allowlisted route with no query expansion; tenant-scoped bounded public projection; aggregate lifecycle counts; no raw events or payloads, prompts, results, artifacts, approvals, capabilities, or authority records; explicit open gaps and non-certification claim; `PROJECTION_AVAILABLE` does not assert effectiveness; SHA-256 export fingerprint; no-store response | the local owner can disclose the downloaded inventory; the fingerprint verifies the export bytes but is not a ledger attestation or conformity assessment |
