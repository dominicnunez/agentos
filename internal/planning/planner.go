...
func (p *ModelPlanner) Build(ctx context.Context, input Input, kind core.ExecutionKind) (Result, error) {
	...
	if input.Strategy != nil {
		request.Messages = append(request.Messages, modelinput.Message{Role: modelinput.Data, Text: string(strategic), Source: modelinput.Source{
			Kind: modelinput.StrategyContext, Reference: string(input.Strategy.Goal.ID), Digest: modelinput.TextDigest(string(strategic)),
		}})
	}
	...
	if len(body) > maximumPromptBytes {
...
func ValidateModelInput(input Input) error {
	...
	// The structured request carries each serialized source as a JSON string.
	// Reserve that second layer of escaping before recording a planning attempt.
	quoted, err := json.Marshal(string(body))
	if err != nil {
		return fmt.Errorf("encode structured planning input: %w", err)
	}
	if len(quoted) > maximumPromptBytes-modelPromptOverhead {
		return fmt.Errorf("complete planning input exceeds the model-prompt limit")
	}
	return nil
}
