...
| Event ledger integrity | PASS | Current storage chains exact event bytes with SHA-256 and binds the verified head plus exact authority-bearing records, consumed approvals, and inference state to an Ed25519-signed checkpoint outside SQLite. Startup, runtime/maintenance single-writer enforcement, recoverable maintenance fencing, configuration serialization, bounded transition retention, diagnostics, paired backup/restore, rollback and substitution rejection, pending-commit recovery, continuity-preserving rotation, explicit key-loss trust reset, credential revocation, and assurance limits are tested and documented. |
