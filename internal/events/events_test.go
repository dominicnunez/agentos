...
func TestAgentKnowledgeAttributionBindsCanonicalCandidatePayload(t *testing.T) {
	knowledgeID := core.ID("knowledge-1")
	title := "Bounded lesson"
	basis := core.KnowledgeBasisSingleExperience
	applicability := "repository work"
	payload := KnowledgeProposedPayload{
		KnowledgeID: &knowledgeID, KnowledgeType: core.KnowledgeLesson, Title: &title,
		Content: "Validate exact contracts.", BasisType: &basis, Applicability: &applicability,
	}
	body, err := json.Marshal(payload)
	if err != nil {
		t.Fatal(err)
	}
	proposal := Event{Payload: body}
	candidate := core.KnowledgeRecord{
		KnowledgeID: knowledgeID, Type: core.KnowledgeLesson, Title: title, Content: payload.Content,
		Basis: basis, Applicability: applicability,
	}
	if !validAgentKnowledgeProposalPayload(proposal, candidate) {
		t.Fatal("canonical proposal was not bound to its exact candidate")
	}
	candidate.Content = "substituted content"
	if validAgentKnowledgeProposalPayload(proposal, candidate) {
		t.Fatal("proposal was attributed to substituted candidate content")
	}
}

func TestRepeatedObservationRequiresPublishedContractAndAuthenticatedExecution(t *testing.T) {
	now := time.Unix(20, 0).UTC()
	payload := EvidencePublishedPayload{Summary: "independent observation", ArtifactRefs: []string{"artifact-1"}}
	body, err := json.Marshal(payload)
	if err != nil {
		t.Fatal(err)
	}
	evidence := Event{
		EventID: "evidence-1", Sequence: 20, OrganizationID: "org-1", EventType: "EVIDENCE_PUBLISHED",
		SourceActorID: "agent-1", SourceExecutionID: "execution-1", TaskID: "task-1",
		ArtifactRefs: []string{"artifact-1"}, Payload: body, CorrelationID: "knowledge-knowledge-1",
		CreatedAt: now, SchemaVersion: SchemaVersion,
	}
	knowledge := core.KnowledgeRecord{
		KnowledgeID: "knowledge-1", OrganizationID: "org-1", ValidationMethod: core.KnowledgeValidationRepeatedObservation,
	}
	if ValidKnowledgeValidationEvidence(evidence, knowledge, 10, nil) {
		t.Fatal("well-formed but unauthenticated evidence activated repeated-observation knowledge")
	}
	evidence.Payload = json.RawMessage(`{"knowledge_id":"knowledge-1","candidate_version":1,"observation":"invented"}`)
	if ValidKnowledgeValidationEvidence(evidence, knowledge, 10, nil) {
		t.Fatal("invented observation contract activated repeated-observation knowledge")
	}
}

func TestKnowledgeJudgmentRequiresStatementSeparateFromAuthorization(t *testing.T) {
	now := time.Unix(30, 0).UTC()
	candidateVersion := 1
	capability := Event{
		EventID: "capability-checked", Sequence: 20, OrganizationID: "org-1", EventType: "CAPABILITY_CHECKED",
		SourceActorID: "reviewer-1", TaskID: "task-validation", CreatedAt: now, SchemaVersion: SchemaVersion,
	}
	knowledge := core.KnowledgeRecord{
		KnowledgeID: "knowledge-1", OrganizationID: "org-1", Version: 2, SupersedesVersion: &candidateVersion, ValidationMethod: core.KnowledgeValidationHuman,
		ValidationRefs: []string{capability.EventID}, ValidatedBy: "reviewer-1", ValidatedByKind: core.PrincipalHuman,
	}
	validator := NewKnowledgeAdmissionValidator([]Event{capability})
	if validator.hasPostProposalValidation(knowledge, 10) {
		t.Fatal("capability authorization was mistaken for a Human judgment")
	}

	input := KnowledgeJudgmentPayload{
		KnowledgeID: "knowledge-1", CandidateVersion: 1, Decision: KnowledgeJudgmentValidated,
	...
	statement := Event{
		...
		SourceActorID: "reviewer-1", TaskID: "task-validation", CorrelationID: "work-validation",
		Payload: body, CreatedAt: now.Add(time.Second), SchemaVersion: SchemaVersion,
	}
	knowledge.ValidationRefs = append(knowledge.ValidationRefs, statement.EventID)
	validator = NewKnowledgeAdmissionValidator([]Event{capability, statement})
	if !validator.hasPostProposalValidation(knowledge, 10) {
		t.Fatal("canonical authenticated Human statement was not accepted as judgment evidence")
	}
	if !validator.hasJudgmentStatementAfter(knowledge, 10, capability) {
		t.Fatal("judgment statement after authorization was not bound to it")
	}
	wrongTask := statement
	wrongTask.TaskID = "task-other"
	validator = NewKnowledgeAdmissionValidator([]Event{capability, wrongTask})
	if validator.hasJudgmentStatementAfter(knowledge, 10, capability) {
		t.Fatal("judgment statement from a different task used task-scoped authorization")
	}
	frozenStatement := statement
	frozenStatement.Sequence = 22
	validator = NewKnowledgeAdmissionValidator([]Event{capability, frozenStatement})
	validator.UseOrganizationFreezeAdmissions([]OrganizationFreezeAdmission{{OrganizationID: "org-1", Frozen: true, Sequence: 21}})
	if validator.hasJudgmentStatementAfter(knowledge, 10, capability) {
		t.Fatal("judgment statement emitted while its organization was frozen was accepted")
	}
	if validator.hasJudgmentStatementAfter(knowledge, 10, statement) {
		t.Fatal("statement recorded before authorization was accepted")
	}
	statement.Payload = json.RawMessage(`{"knowledge_id":"knowledge-1","candidate_version":1,"decision":"VALIDATED","statement":"I validate it","capability_check_event_id":"capability-checked","source_principal_id":"other","source_principal_kind":"HUMAN","source_channel":"HUMAN_DIRECT","artifact_refs":[]}`)
	validator = NewKnowledgeAdmissionValidator([]Event{capability, statement})
	if validator.hasPostProposalValidation(knowledge, 10) {
		t.Fatal("statement with substituted source identity was accepted")
	}

	genericPayload, err := json.Marshal(OperatorInputReceivedPayload{
		MessageID:         "message-generic",
		SourcePrincipalID: "reviewer-1", SourcePrincipalKind: string(core.PrincipalHuman), SourceChannel: "HUMAN_DIRECT",
		Text: "I independently validate this candidate.",
	})
	if err != nil {
		t.Fatal(err)
	}
	generic := statement
	generic.EventID = "generic-input"
	generic.EventType = "HUMAN_INPUT_RECEIVED"
	generic.Payload = genericPayload
	knowledge.ValidationRefs = []string{capability.EventID, generic.EventID}
	validator = NewKnowledgeAdmissionValidator([]Event{capability, generic})
	if validator.hasPostProposalValidation(knowledge, 10) {
		t.Fatal("generic Human input was accepted as an exact knowledge judgment")
	}

	for _, test := range []struct {
		name   string
		mutate func(*KnowledgeJudgmentPayload)
	}{
		{name: "different knowledge", mutate: func(payload *KnowledgeJudgmentPayload) { payload.KnowledgeID = "knowledge-other" }},
		{name: "different candidate version", mutate: func(payload *KnowledgeJudgmentPayload) { payload.CandidateVersion = 2 }},
		{name: "different capability check", mutate: func(payload *KnowledgeJudgmentPayload) { payload.CapabilityCheckEventID = "capability-other" }},
		{name: "oversized statement", mutate: func(payload *KnowledgeJudgmentPayload) { payload.Statement = strings.Repeat("x", (16<<10)+1) }},
		{name: "A2A identity on Human boundary", mutate: func(payload *KnowledgeJudgmentPayload) {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			knowledge.ValidationRefs = []string{capability.EventID, candidate.EventID}
			validator := NewKnowledgeAdmissionValidator([]Event{capability, candidate})
			if validator.hasJudgmentStatementAfter(knowledge, 10, capability) ||
				test.name != "different capability check" && validator.hasPostProposalValidation(knowledge, 10) {
				t.Fatal("malformed or substituted knowledge judgment was accepted")
			}
		})
	}
}
...
func TestKnowledgeEvidenceArtifactsMustBelongToCandidate(t *testing.T) {
	...
	if knowledgeEvidenceArtifactsBound([]string{"artifact-forged"}, []string{"artifact-1"}) {
		t.Fatal("evidence artifact outside the candidate was accepted")
	}
}
...
func TestKnowledgeAuthorityRequiresExactDurableRecords(t *testing.T) {
	...
	stream := []Event{{EventID: "grant-1", Sequence: 1, OrganizationID: "org-1", EventType: "CAPABILITY_GRANTED", SourceActorID: "runtime", TaskID: "task-1", Payload: body, CreatedAt: now, SchemaVersion: SchemaVersion}}
	if _, _, err := ResolveAuthorityAdmissions(stream, nil); err == nil {
		t.Fatal("bare capability event became replay authority")
	}
	leases, freezes, err := ResolveAuthorityAdmissions(stream, []AuthorityRecord{{Kind: "capability_lease", RecordID: "lease-1", Version: 1, Body: body, AdmissionEventID: "grant-1"}})
...
func TestAgentEvidenceAndKnowledgeProposalsRequirePublishedContracts(t *testing.T) {
	ledger := &memoryLedger{}
	gateway := NewGateway(ledger)
	evidence := Draft{
		EventType: "EVIDENCE_PUBLISHED", TaskID: "task-1", ArtifactRefs: []string{"artifact-1"},
		Payload: EvidencePublishedPayload{Summary: "independent observation", ArtifactRefs: []string{"artifact-1"}},
	}
	if _, err := gateway.PublishAgentDraft(context.Background(), "org", "agent", "execution", "knowledge-k-1", evidence); err != nil {
		t.Fatal(err)
	}
	invalidEvidence := evidence
	invalidEvidence.Payload = map[string]any{"knowledge_id": "k-1", "observation": "invented contract"}
	if _, err := gateway.PublishAgentDraft(context.Background(), "org", "agent", "execution", "knowledge-k-1", invalidEvidence); err == nil {
		t.Fatal("invented evidence payload bypassed the published contract")
	}

	knowledgeID := core.ID("k-1")
	title := "Bounded lesson"
	basis := core.KnowledgeBasisSingleExperience
	applicability := "repository work"
	proposal := Draft{
		EventType: "KNOWLEDGE_PROPOSED", TaskID: "task-1",
		Payload: KnowledgeProposedPayload{
			KnowledgeID: &knowledgeID, KnowledgeType: core.KnowledgeLesson, Title: &title,
			Content: "Validate exact contracts.", BasisType: &basis, Applicability: &applicability,
		},
	}
	if _, err := gateway.PublishAgentDraft(context.Background(), "org", "agent", "execution", "work-1", proposal); err != nil {
		t.Fatal(err)
	}
	invalidProposal := proposal
	invalidProposal.Payload = map[string]string{"summary": "legacy proposal"}
	if _, err := gateway.PublishAgentDraft(context.Background(), "org", "agent", "execution", "work-1", invalidProposal); err == nil {
		t.Fatal("noncanonical knowledge proposal bypassed the published contract")
	}
	invalidProposal.Payload = json.RawMessage(`{"knowledge_type":"LESSON","content":"Validate exact contracts.","occurrence_event_refs":null}`)
	if _, err := gateway.PublishAgentDraft(context.Background(), "org", "agent", "execution", "work-1", invalidProposal); err == nil {
		t.Fatal("null proposal array bypassed the published contract")
	}
	if len(ledger.events) != 2 {
		t.Fatalf("invalid Agent contracts reached the ledger: %+v", ledger.events)
	}
}
