...
def send_message(url: str, token: str, rpc_id: str, message_id: str, context_id: str, text: str) -> dict:
    response = load_json(
        url.rstrip("/") + "/",
        token,
        {
...
def main() -> int:
    ...
    deterministic = send_message(
        args.url, args.token, "rpc-1", "message-1", "a2a-deterministic", "echo hello-from-agent"
    )
