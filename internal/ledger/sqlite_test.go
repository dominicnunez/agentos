...
func TestApprovalConsumptionAndAttemptTransitionAreAtomic(t *testing.T) {
	...
	_, err = l.AuthorizeAndAppendEffectAttempt(ctx, "org-1", "task-1", "actor-1", "send", "customer-1", "org-1", []string{"lease-1"}, "approval-1", "fingerprint-1", true, nil, "effect", "effect-1", 2, attempted)
	if err == nil {
		t.Fatal("injected attempt failure was ignored")
	}
	stream, err := l.Events(ctx, "")
	if err != nil || len(stream) != 2 || stream[1].EventType != "EFFECT_OBLIGATION_TRANSITIONED" {
	...
	if _, err := l.AuthorizeAndAppendEffectAttempt(ctx, "org-1", "task-1", "actor-1", "send", "customer-1", "org-1", []string{"lease-1"}, "approval-1", "fingerprint-1", true, nil, "effect", "effect-1", 2, attempted); err != nil {
		t.Fatalf("rolled-back approval could not be retried: %v", err)
	}
	if _, err := l.AuthorizeAndAppendEffectAttempt(ctx, "org-1", "task-1", "actor-1", "send", "customer-1", "org-1", []string{"lease-1"}, "approval-1", "fingerprint-1", true, nil, "effect", "effect-2", 1, attempted); err == nil {
