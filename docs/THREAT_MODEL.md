...
| Semantic intake | invented operator choices, hidden Goal or Lab-mode substitution, confirmation replay, Goal lifecycle race | strict bounded output; explicit `STANDARD`/`EXPERIMENT` mode in the complete draft fingerprint; runtime-owned Lab containment only; unsupported adaptive experiments rejected before confirmation; explicit source-message provenance for Goal IDs; exact-ID presence check; active same-tenant Goal admission in the confirmation transaction; immutable Intent and Work Goal binding; later Goal changes cannot strand admitted Work | an authorized operator can explicitly confirm the wrong active Goal or reviewed mode |
