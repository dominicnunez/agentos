...
Incident validation privately follows projection dependencies and checks retained lifecycle admissions and completion evidence with shared recovery validators. Selected capability grants/revocations and connection-policy replacements are included when validating authority and inference admissions. Exact record backing and aggregate loading bounds constrain that supporting evidence; it does not enter the public report. A history can exceed those bounds even when the selected timeline is small. Complete-chain verification still scans retained ledger history per request.
