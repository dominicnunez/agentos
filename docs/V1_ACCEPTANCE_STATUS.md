...
| 18 | Hermes discovers, submits, and continues through A2A | Partial | Agent Card, authenticated submission, status, and input persistence are tested; `TestA2AStatusAndInputContinuation` proves authorized, idempotent continuation and rejects non-HUMAN replay, while `TestRecoverCompletesDurableExternalInputExactlyOnce` covers interruption before resume, after resume, and after verification. Pinned real Hermes interoperability and result Artifact mapping remain. |
