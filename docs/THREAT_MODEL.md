...
| Semantic intake | invented operator choices, hidden Goal or Lab-mode substitution, confirmation replay, Goal lifecycle race | strict bounded output; explicit `STANDARD`/`EXPERIMENT` mode in the complete draft fingerprint; runtime-owned Lab containment only; unsupported adaptive experiments rejected before confirmation; explicit source-message provenance for Goal IDs; exact-ID presence check; active same-tenant Goal admission in the confirmation transaction; immutable Intent and Work Goal binding | an authorized operator can explicitly confirm the wrong active Goal or reviewed mode |
...
| Agent dispatch | deactivation race, changed organizational direction, stale or substituted Agent configuration, cross-tenant roster reference, replayed start authority | current Mission/Goal revisions must still match the immutable Plan or the Task blocks; execution input and manifest bind exact strategic events and versions; one SQLite transaction reloads the exact active Agent, blueprint, and execution profile while consuming the pending Task revision; sealed start detail binds projection versions and admission events; replay and recovery reject stale, superseded, malformed, or cross-organization bindings; these bindings grant no capability or effect authority | a dispatch committed before deactivation or strategic revision may finish; interrupted adaptive provider contact remains uncertain |
