...
| Model completion review | Agent self-certification, stale review, result disclosure, ephemeral-origin loss | owner-only private control; exact candidate/evidence fingerprint; no-store responses; durable idempotent decision and recovery; bounded authorized terminal-decision projection | the user's subjective judgment can be wrong |
