	...
	for _, mode := range []string{"fresh", "upgrade-v12"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "upgrade-v12" {
				...
				if _, err := db.ExecContext(t.Context(), `DROP INDEX IF EXISTS records_replaced_work_idx; DROP INDEX IF EXISTS events_incident_execution_idx;
INSERT INTO records(kind,record_id,version,body,created_at) VALUES('retained','malformed',1,?,'2026-09-22T00:00:00Z');`, []byte(`{`)); err != nil {
					t.Fatal(err)
				}
...
func TestIncidentIndexesRequired(t *testing.T) {
	for _, index := range []string{"records_replaced_work_idx", "events_incident_execution_idx"} {
