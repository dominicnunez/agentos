...
EVIDENCE_FILES = {
    ...
    "docs/governance/ai-management-system.md": "aims-technical-evidence-boundary",
    "docs/guides/approval-control.md": "user-authority",
    "docs/security/event-ledger-integrity.md": "evidence-integrity",
    "docs/operations/governance-inspection.md": "monitoring-and-inspection",
    "docs/operations/incident-replay.md": "incident-investigation",
    "docs/architecture/lab.md": "governed-experimentation",
    "docs/architecture/organizational-knowledge.md": "organizational-memory",
    "docs/architecture/shared-coordination.md": "shared-coordination",
    "docs/operations/sqlite-recovery.md": "resilience-and-recovery",
    "docs/threat-model.md": "ai-and-security-risk",
    "docs/development/build-contract.md": "lifecycle-controls",
    "docs/development/v1-acceptance-status.md": "acceptance-evidence",
}
...
def _git_aims_entries(repo_root: Path, commit: str) -> dict[str, bytes]:
    ...
    for raw_entry in listing.split(b"\0"):
        ...
        if mode != "100644" or object_type != "blob":
            raise VerificationError(
                f"Git AIMS entry must be a regular non-executable file: {relative}"
            )
        path = PurePosixPath(relative)
        if (
            not relative
...
def _verify_aims_history(
    ...
    for revision in revisions:
        ...
        parents_by_commit[revision] = parent_line[1:]
        boundary_parents.update(
            parent for parent in parent_line[1:] if parent not in revision_set
        )
...
def readiness_report(
    ...
    outcomes = manifest.get("assessment_outcomes")
    if outcomes is None:
        blockers.append("AFFIRMATIVE_ASSESSMENT_OUTCOMES_NOT_RECORDED")
    else:
        audit = outcomes["audit"]
        if audit["result"] != "PASS" or audit["open_blocking_findings"] != 0:
