...
| Consequential effects | duplicate action, cross-tenant or malformed lease use, malformed freeze/revocation history, crash after send, false success | persist-before-effect obligation; closed capability/freeze Event Contracts with exact atomic records, tenant-bound transactional resolution, contiguous immutable grant-to-revocation history, deterministic recovery validation, exact approval checks, single-use consumption, idempotency key, evidence-required confirmation, no blind resend | production effect-writing adapters remain absent |
