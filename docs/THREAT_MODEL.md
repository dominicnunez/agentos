...
| A2A protocol | method confusion, result disclosure, compatibility shims that widen access | canonical A2A v1.0 methods only; capability-separated status/result access; generic not-found behavior across tenant boundaries; no legacy aliases | protocol evolution requires a new review and interoperability evidence |
...
| Release pipeline | untracked source, dependency substitution, non-reproducible output, artifact mix-up | exact clean commit; pinned Go/Python and actions; module hash verification; CGO/VCS stripping; two independent builds and byte comparison; checksums, target SBOMs, provenance; packaged-binary pilot | provenance is unsigned and artifacts are not published until a separately approved release workflow exists |
